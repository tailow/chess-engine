...
#define CBPAWN 15   // 1111
    ...
    for (unsigned i = 0; i < nbrof(dst.ints); i++)
    {
        big_hash ^= dst.ints[i];
    }
...
int ChessRules::GetRepetitionCount()
{
    ...
    // Search backwards ....
    unsigned int nbr_half_moves = (full_move_count - 1) * 2 + (!white ? 1 : 0);
    ...
    for (unsigned int i = 0; i < nbr_half_moves; i++)
    {
        Move m = history[--idx];
        if (m.src == m.dst)
...
#define BONUS_STRONG_KING 50
    ...
    if (score_white_pieces == 0)
    {
        ...
        while (black_passers > black_passers_buf && white_king_square != SQUARE_INVALID && !black_will_queen)
        {
            --black_passers;
...
//  easily winning pawn endings
        ...
        while (score_white_material == 500 && planning_score_white_pieces && black_pawns > black_pawns_buf && !black_will_queen)
        {
...
#ifdef USE_IN_THE_SQUARE
        while (white_passers > white_passers_buf && black_king_square != SQUARE_INVALID && !white_will_queen)
        {
            --white_passers;
...
//  easily winning pawn endings
#ifdef USE_LIQUIDATION
        while (score_black_material == -500 && planning_score_black_pieces && white_pawns > white_pawns_buf && !white_will_queen)
        {
            int nbr_separating_files = (!white ? 5 : 4);
            --white_pawns;
